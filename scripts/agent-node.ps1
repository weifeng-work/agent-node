# =====================================================================
# agent-node 启动/停止/状态/重启/更新 —— 无 Node 版本（纯 PowerShell）
# 由 agent-node.cmd 转发调用；不依赖 npm/Node/系统 Python。
# =====================================================================
[CmdletBinding()]
param(
...
function Show-Line([string]$s) { Write-Host $s -ForegroundColor Cyan }
...
function Get-PanelUrl {
    if (Test-Path $URL) { return ((Get-Content $URL -Raw).Trim()) }
...
function Start-Node {
    if (Is-Running) {
        Write-Host "节点已在运行，打开面板…"
        Open-Panel
        return
    }
    if (-not (Test-Path $PYW)) {
        Write-Host ("未安装或缺少运行时 Python: " + $PYW) -ForegroundColor Red
