...
**不需要 Node / npm / git**。脚本自动检测系统 Python（≥3.10，缺失用 winget 装一次）→
下载源码 zip → 在 `%LOCALAPPDATA%\agent-node` 建 venv 装依赖 → 确保 `data\`
（已存在则保留，**重装/更新不丢身份与配置**）→ 安装全局命令 `agent-node`（纯 PowerShell，
加入用户 PATH）→ **创建桌面快捷方式** → **自动启动节点并打开面板**。
