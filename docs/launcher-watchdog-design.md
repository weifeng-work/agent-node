...
> **v2 进度**：更新检查（D3）已落地 v0.2.4——为**托盘右键"检查更新"**（见 §7.3 菜单），非原设计的"启动自动检查"；其余 v2 项（联机开关/优雅停止/run_as_admin）仍后置。
...
| D3(已落地 v2) | **更新：托盘"检查更新"，只提示** | 查询 GitHub 最新 Release tag，与本地已装节点 `node/version.py` 版本比较；有新版本托盘弹窗"发现新版本 vX，是否更新"，用户确认才跑 install.ps1。默认不自动更新。 | v2 |
...
> v2 追加：联机开关。更新检查（D3）已落地本菜单：`打开面板 / 启动节点 / 停止节点 / 重启节点 / 检查更新 / 退出托盘`。
...
- **v1 更新 = 用户手动跑 install.ps1**（D3 后置）。**v0.2.4 起改为托盘"检查更新"**（见下 §8.2）。

### 8.2 更新检查（D3，已落地 v0.2.4）

- **版本对比**：本地 = 已安装 `{ROOT}\app\node\version.py` 的 `VERSION` 常量（部署形态被 bump 写为当前 tag）；远端 = `api.github.com/repos/weifeng-work/agent-node/releases/latest` 的 `tag_name`。二者均剥前导 `v`，用 `versionInts` 三元组比较。
- **触发**：托盘右键 **"检查更新"**（非原"启动自动检查"——手动、按需、无后台轮询）。
- **流程**：点击 → 查远端/本地 → 有新版本弹 MessageBox"发现新版本 vX（当前 vY），是否立即更新？" → **确认才跑 `runInstallScript`**（与 D6 同一 install.ps1）；无更新/失败/本地超前均给明确提示。
- **只提示、不自动更新**（D3 原则）：默认不下载，杜绝后台静默改动。
- **不阻塞看门狗**：网络查询与确认弹窗放在 goroutine，run 循环的 3s reconcile 不被拖住。
...
- ~~更新检查~~（**D3 已提前落地 v0.2.4**，见 §8.2：托盘"检查更新"手动触发 + 提示；"启动自动轮询 + 更新前停机协调"仍未做）。
